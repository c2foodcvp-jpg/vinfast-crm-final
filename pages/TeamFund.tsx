...
const TeamFund: React.FC = () => {
    ...
    // Filter Data based on selectedTeam AND Date
    const filteredFines = useMemo(() => {
        return fines.filter(f => {
            if (!isInMonthYear(f.created_at)) return false;
            if (!isAdmin || selectedTeam === 'all') return true;
            const user = employees.find(e => e.id === f.user_id);
            return user?.manager_id === selectedTeam || user?.id === selectedTeam;
        });
    }, [fines, selectedTeam, employees, isAdmin, selectedMonth, selectedYear]);

    const filteredExpenses = useMemo(() => {
        return expenses.filter(e => {
            if (!isInMonthYear(e.created_at)) return false;
            if (!isAdmin || selectedTeam === 'all') return true;
            const user = employees.find(emp => emp.id === e.created_by);
            return user?.manager_id === selectedTeam || user?.id === selectedTeam;
        });
    }, [expenses, selectedTeam, employees, isAdmin, selectedMonth, selectedYear]);
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div className="flex flex-col md:flex-row items-center gap-2">
                    <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-xl px-3 py-1.5 shadow-sm">
                        <div className="flex items-center gap-1 text-gray-500 text-sm font-bold mr-2 border-r border-gray-200 pr-3">
                            <History size={16} /> Lọc
                        </div>
                        <span className="text-gray-500 text-sm">Tháng</span>
                        <select value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value === 'all' ? 'all' : Number(e.target.value))} className="text-sm font-bold text-gray-700 bg-transparent outline-none cursor-pointer">
                        ...
                        <select value={selectedYear} onChange={(e) => setSelectedYear(Number(e.target.value))} className="text-sm font-bold text-gray-700 bg-transparent outline-none cursor-pointer">
