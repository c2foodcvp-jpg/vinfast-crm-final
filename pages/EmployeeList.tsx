...
import { CheckCircle, XCircle, Search, Shield, User, Edit2, Save, X, AlertTriangle, ShieldCheck, Users, Copy, Terminal, Trash2, Clock, LogOut, ArrowRightLeft, Share2, Database, Eye } from 'lucide-react';
...
const EmployeeList: React.FC = () => {
  ...
  const handleDeleteUser = async (id: string) => {
      if (!window.confirm("CẢNH BÁO: Bạn có chắc chắn muốn xóa nhân viên này?")) return;
      const originalEmployees = [...employees];
  ...
  const handleEndPartTimeContract = async (employee: UserProfile) => {
      if (!employee.manager_id) { alert("Lỗi: Nhân viên này không có người quản lý!"); return; }
      const manager = employees.find(e => e.id === employee.manager_id);
      if (!manager) { alert("Lỗi: Không tìm thấy người quản lý!"); return; }
      if (!window.confirm(`XÁC NHẬN: Kết thúc hợp đồng Part-time của "${employee.full_name}"?`)) return;

      const originalEmployees = [...employees];
      setEmployees(employees.map(e => e.id === employee.id ? { ...e, status: 'blocked' } : e));

      try {
          await supabase.from('customers').update({ creator_id: manager.id, sales_rep: manager.full_name }).eq('creator_id', employee.id);
          await supabase.from('profiles').update({ status: 'blocked' }).eq('id', employee.id);
          await supabase.from('interactions').insert([{ user_id: userProfile?.id, customer_id: null, type: 'note', content: `Hệ thống: Đã kết thúc HĐ Part-time của ${employee.full_name}.`, created_at: new Date().toISOString() }]);
          alert("Đã hoàn tất!");
      } catch (err: any) {
          setEmployees(originalEmployees);
                      ...
                      <tr key={emp.id} className="hover:bg-gray-50 transition-colors">
                          <td className="px-6 py-4">
                              <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                      <div className="flex items-center gap-2"><p className="font-semibold text-gray-900 cursor-pointer hover:text-primary-600" onClick={() => (isAdmin || isMod) && emp.status === 'active' && window.location.assign('#/employees/' + emp.id)}>{emp.full_name}</p>{emp.is_part_time && (<span className="px-1.5 py-0.5 bg-orange-100 text-orange-700 text-[9px] font-bold rounded uppercase border border-orange-200">Part-time</span>)}</div>
                          ...
                          <td className="px-6 py-4 text-right">
                            {emp.status === 'pending' && (isAdmin || isMod) ? (<div className="flex justify-end gap-2"><button onClick={() => openApprovalModal(emp)} className="flex items-center gap-1 px-3 py-1.5 bg-green-600 text-white rounded-lg text-xs font-bold hover:bg-green-700 shadow-md shadow-green-200 transition-all active:scale-95"><CheckCircle size={14} /> Duyệt</button><button onClick={() => handleReject(emp.id)} className="flex items-center gap-1 px-3 py-1.5 bg-white text-red-600 border border-red-200 rounded-lg text-xs font-bold hover:bg-red-50 transition-all active:scale-95"><XCircle size={14} /> Từ chối</button></div>) : isAdmin && (<div className="flex justify-end gap-2">{emp.is_part_time && emp.status === 'active' && (<button onClick={() => handleEndPartTimeContract(emp)} className="flex items-center gap-1 px-3 py-1.5 bg-orange-100 text-orange-700 border border-orange-200 rounded-lg text-xs font-bold hover:bg-orange-200" title="Kết thúc HĐ & Chuyển khách về MOD"><ArrowRightLeft size={14} /> Kết thúc HĐ</button>)}<button onClick={() => handleDeleteUser(emp.id)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Xóa nhân viên"><Trash2 size={18} /></button></div>)}
                          </td>
                      </tr>
                  ))
              )}
      ...
      {showDelegateModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
              <div className="bg-white rounded-2xl w-full max-w-2xl p-6 shadow-2xl overflow-hidden max-h-[90vh] flex flex-col relative">
                  ...
                  {delegationToDelete && (
                      <div className="absolute inset-0 bg-white/95 backdrop-blur-sm z-[60] flex flex-col items-center justify-center p-6 text-center animate-fade-in">
...
on profiles for update
to authenticated
using ( auth.uid() = id );`; navigator.clipboard.writeText(code); alert("Đã sao chép SQL!"); }} className="absolute top-3 right-3 p-2 bg-gray-700 hover:bg-gray-600 text-white rounded-lg transition-colors flex items-center gap-2 text-xs font-bold"><Copy size={14} /> Sao chép</button></div></div></div></div>)}
