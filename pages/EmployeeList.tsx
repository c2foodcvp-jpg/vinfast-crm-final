...
import { CheckCircle, XCircle, Search, Shield, User, Edit2, Save, X, AlertTriangle, ShieldCheck, Users, Copy, Terminal, Trash2, Clock, LogOut, ArrowRightLeft, Share2, Database, Eye, Lock, UserMinus, Key } from 'lucide-react';
          ...
          <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
              <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl border-2 border-gray-100">
                  <div className="flex flex-col items-center text-center">
                      <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 ${confirmAction.type === 'terminate' ? 'bg-red-100 text-red-600' : confirmAction.type === 'delete' ? 'bg-red-100 text-red-600' : 'bg-orange-100 text-orange-600'} animate-pulse`}>
                          {confirmAction.type === 'terminate' ? <UserMinus size={32} /> : confirmAction.type === 'delete' ? <Trash2 size={32} /> : <AlertTriangle size={32} />}
                      </div>
                      ...
                      <div className="flex gap-3 w-full">
                          ...
                          <button onClick={executeConfirmAction} className={`flex-1 py-3 text-white font-bold rounded-xl shadow-lg transition-colors ${confirmAction.type === 'terminate' || confirmAction.type === 'delete' ? 'bg-red-600 hover:bg-red-700 shadow-red-200' : 'bg-orange-600 hover:bg-orange-700 shadow-orange-200'}`}>
                              Xác nhận
                          </button>
                      </div>
                  </div>
...
on profiles for update
to authenticated
using ( auth.uid() = id );`}</pre><button onClick={() => { const code = `-- 1. Bổ sung cột phân quyền hạn chế (Fix lỗi 400 Bad Request)
alter table profiles add column if not exists is_locked_add boolean default false;
alter table profiles add column if not exists is_locked_view boolean default false;
