...
const LeadsFromForm: React.FC = () => {
  ...
  const handleSelfAssign = async (lead: Customer) => {
    if (!userProfile) return;
    if (!window.confirm(`Bạn có chắc muốn nhận khách hàng "${lead.name}" về danh sách của mình?`)) return;

    setIsAssigning(true);

    try {
      const { error } = await supabase.from('customers').update({ sales_rep: userProfile.full_name, creator_id: userProfile.id }).eq('id', lead.id);
      if (error) throw error;

      await supabase.from('interactions').insert([{
        customer_id: lead.id, user_id: userProfile.id, type: 'note',
      ...
      const errorMessage = err?.message || (typeof err === 'object' ? JSON.stringify(err) : String(err));
      alert("Lỗi nhận khách: " + errorMessage);
    } finally { setIsAssigning(false); }
  };
