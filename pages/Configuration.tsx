...
const VersionCheckButton: React.FC = () => {
    ...
    const handleCheck = async () => {
        ...
        if (result && !hasUpdate) {
            alert(`Hệ thống đang ở phiên bản mới nhất! (Build: ${result.buildTime})`);
        }
    };

    if (hasUpdate) {
        return (
            <button onClick={reload} className="flex items-center gap-2 bg-red-600 text-white px-3 py-2 rounded-xl text-sm font-bold shadow-sm hover:bg-red-700 animate-pulse">
    ...
    return (
        <button onClick={handleCheck} disabled={isChecking} className="flex items-center gap-2 bg-white border border-gray-300 text-gray-700 px-3 py-2 rounded-xl text-sm font-bold shadow-sm hover:bg-gray-50 disabled:opacity-50">
