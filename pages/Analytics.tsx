...
const Analytics: React.FC = () => {
  ...
  const filteredData = useMemo(() => {
      ...
      const fCustomers = customers.filter(c => {
          // Use deal closing date or update date for revenue timing
      ...
      const fTransactions = transactions.filter(t => {
          ...
          // If transaction has a customer_id, that customer MUST be MKT Group to be counted in "Net MKT"
          if (t.customer_id) {
              const cust = customers.find(c => c.id === t.customer_id);
              if (cust && !cust.source?.includes('MKT')) {
                  return false; // Skip non-MKT transactions
              }
          }
          return true;
      });
  ...
  const kpis = useMemo(() => {
      ...
      const projectedRevenue = cust
          .filter(c => c.status === CustomerStatus.WON && c.source?.includes('MKT'))
          .reduce((sum, c) => sum + (c.deal_details?.revenue || 0), 0);

      // 2. Real Revenue (Thực thu) - Matches Finance 'pnlRevenue'
      const revenue = trans
          .filter(t => ['deposit', 'adjustment'].includes(t.type) && t.amount > 0)
          .reduce((sum, t) => sum + t.amount, 0);

      // 3. Real Expenses (Chi phí thực) - Matches Finance 'realExpenses' (Excludes Advances)
      const expenses = trans
          .filter(t => t.type === 'expense' || (t.type === 'adjustment' && t.amount < 0))
          .reduce((sum, t) => sum + Math.abs(t.amount), 0);

      // 4. Part-time Salary Liability - Matches Finance 'partTimeSalaryLiability'
      const partTimeLiability = trans
          .filter(t => {
              if (t.type !== 'deposit') return false;
              const creator = teamMembers.find(m => m.id === t.user_id);
              return creator?.is_part_time;
          })
          .reduce((sum, t) => sum + (t.amount * 0.3), 0); // 30% rule

      // 5. Net Profit (Lợi nhuận ròng) - Matches Finance 'pnlNet'
      // Formula: Revenue - Real Expenses - PartTime Salary
      // Note: Advances are NOT subtracted here (they are cashflow, not P&L)
      const netProfit = revenue - expenses - partTimeLiability;
      
      const totalCost = expenses + partTimeLiability;

      // 6. Debt (For display only)
      const customerDeposits = trans.filter(t => t.type === 'deposit' && t.customer_id).reduce((sum, t) => sum + t.amount, 0);
  ...
  const trendData = useMemo(() => {
      ...
      if (isYearly) {
          ...
          // Use a map for daily grouping
          // ... (simplified loop for daily)
      }

      // Helper to add data
      const addToBucket = (date: Date, inc: number, exp: number) => {
          let idx = 0;
          if (isYearly) idx = date.getMonth();
          // For daily, logic needs map, but let's stick to simple array for now if strictly yearly/monthly
          // Implementing simple map for daily/monthly generic
      ...
      trans.forEach(t => {
          ...
          if (['deposit', 'adjustment'].includes(t.type) && t.amount > 0) {
              ...
              // Check part time for expense
              const creator = teamMembers.find(m => m.id === t.user_id);
              if (creator?.is_part_time) {
                  addToBucket(d, 0, t.amount * 0.3);
              }
          }
      });
      ...
      return data;
  }, [filteredData, timeRange, teamMembers]);
