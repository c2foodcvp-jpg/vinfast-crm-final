...
import { Transaction, Customer, UserRole, CustomerStatus, UserProfile } from '../types';
...
import { BadgeDollarSign, Wallet, ArrowUpRight, ArrowDownLeft, CheckCircle2, XCircle, Clock, QrCode, Upload, Save, X, Filter, Settings2, Building2, Calendar, Loader2, Trash2, AlertTriangle } from 'lucide-react';
...
const Finance: React.FC = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // 1. Fetch Profiles to identify Part-timers
      const { data: profileData } = await supabase.from('profiles').select('id, is_part_time');
      ...
      const mappedTransactions: ExtendedTransaction[] = (transData as Transaction[]).map(t => {
          ...
          return { 
              ...t, 
              _source: c?.source || 'Khác',
              _is_part_time_creator: creator?.is_part_time || false
          };
      });
  ...
  // ... (Keep existing handlers: handleApprove, confirmDeleteTransaction, handleSaveQr, handleSubmitDeposit, handleSubmitAdjustment) ...
  ...
  const handleSaveQr = async () => {
      ...
      setIsSavingQr(true);
      try {
          await supabase.from('app_settings').upsert({ key: 'finance_qr_url', value: newQrUrl });
          setQrCodeUrl(newQrUrl);
          setShowConfigModal(false);
          alert("Đã lưu cấu hình QR thành công!");
      } catch (e: any) { alert("Lỗi lưu QR: " + e.message); } finally { setIsSavingQr(false); }
  };
  ...
  const handleSubmitDeposit = async () => {
      ...
      try {
          ...
          fetchData();
          alert(status === 'pending' ? "Đã gửi yêu cầu nộp tiền." : "Đã nộp quỹ thành công!");
  ...
  const handleSubmitAdjustment = async () => {
      ...
      if (isNaN(numAmount) || numAmount === 0) { alert("Vui lòng nhập số tiền hợp lệ."); return; }
      if (!adjustmentForm.reason.trim()) { alert("Vui lòng nhập lý do điều chỉnh."); return; }
      try {
          const payload = {
              user_id: userProfile?.id, user_name: userProfile?.full_name, type: 'adjustment',
              amount: numAmount, reason: `[Điều chỉnh] ${adjustmentForm.reason}`,
              status: 'approved', approved_by: userProfile?.id
          };
      ...
      } catch (e: any) { alert("Lỗi điều chỉnh: " + e.message); }
  };

  // --- FILTERING LOGIC (GMT+7) ---
  ...
  // 2. Total Out (Expenses + Non-deductible Advances)
  const totalOut = filteredTransactions
      .filter(t => t.status === 'approved')
      .filter(t => {
          if (t.type === 'expense') return true;
          if (t.type === 'advance') return t.subtype !== 'deductible';
          return false;
      })
      .reduce((sum, t) => sum + t.amount, 0);

  // 3. PART-TIME SALARY DEDUCTION LOGIC
  // Logic: For every Approved Revenue/Deposit generated by a Part-time employee, 30% is owed to them.
  // This should be treated as a "Virtual Expense" or "Payable" that reduces the Company's available fund.
  // Note: We calculate this on the *Net Revenue* of that specific transaction if possible, but simplistically on the Revenue amount here as per requirement "30% revenue".
  // Prompt says: "trừ đi 30% doanh thu thực tế".
  // We'll iterate through filtered approved deposits/revenues created by part-timers.
  
  const partTimeSalaryLiability = filteredTransactions
      .filter(t => t._is_part_time_creator && ['deposit', 'revenue'].includes(t.type) && t.status === 'approved')
      .reduce((sum, t) => sum + (t.amount * 0.3), 0);

  // Fund Remaining = Total In - Total Out - PartTime Liability
  ...
  const netRevenueDisplay = totalIn - totalExpensesOnly - partTimeSalaryLiability;

  const pieData1 = [
      { name: 'Tồn quỹ công ty', value: fundRemaining > 0 ? fundRemaining : 0 },
      { name: 'Đã chi/ứng', value: totalOut },
      { name: 'Lương Part-time (30%)', value: partTimeSalaryLiability }
  ];
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-20">
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4">
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2"><BadgeDollarSign className="text-green-600"/> Quỹ & Thu Chi (GMT+7)</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center">
              <h3 className="font-bold text-gray-700 mb-4">Tổng quan Quỹ Công ty (Thực)</h3>
              ...
              <div className="text-center mt-2"><p className="text-sm text-gray-500">Quỹ tồn đọng (Sau trừ lương Part-time)</p><p className="text-2xl font-bold text-green-600">{formatCurrency(fundRemaining)} VNĐ</p></div>
              {partTimeSalaryLiability > 0 && <div className="text-center mt-2 text-xs text-orange-600 font-bold bg-orange-50 px-2 py-1 rounded">Dự chi lương Part-time: {formatCurrency(partTimeSalaryLiability)} VNĐ</div>}
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col items-center justify-center gap-4">
              ...
              <div className="text-center w-full"><p className="text-gray-500 font-bold uppercase text-xs">Tổng Chi phí & Lương Part-time</p><p className="text-2xl font-bold text-red-600">-{formatCurrency(totalExpensesOnly + partTimeSalaryLiability)}</p></div>
              <div className="w-full h-px bg-gray-100"></div>
              <div className="text-center bg-blue-50 w-full py-2 rounded-xl border border-blue-100"><p className="text-blue-800 font-bold uppercase text-xs">Lợi nhuận ròng (Net)</p><p className="text-xl font-bold text-blue-900">{formatCurrency(netRevenueDisplay)} VNĐ</p></div>
          </div>
      </div>
              ...
              <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl border border-red-100">
                  <div className="flex flex-col items-center text-center">
                      ...
                      <p className="text-sm text-gray-500 mb-4">
                          Bạn có chắc chắn muốn xóa giao dịch này khỏi hệ thống?
                      </p>
                      <div className="flex gap-3 w-full">
                          <button onClick={() => setTransactionToDelete(null)} className="flex-1 py-3 bg-gray-100 text-gray-700 font-bold rounded-xl hover:bg-gray-200 transition-colors">Hủy bỏ</button>
                          <button onClick={confirmDeleteTransaction} className="flex-1 py-3 bg-red-600 text-white font-bold rounded-xl hover:bg-red-700 shadow-lg shadow-red-200 transition-colors">Xóa ngay</button>
                      </div>
                  </div>
              </div>
          </div>
