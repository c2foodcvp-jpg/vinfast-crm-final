...
import { Edit, Save, Loader2, Info, CarFront } from 'lucide-react';
...
const CarPrices: React.FC = () => {
  ...
  const fetchContent = async () => {
    setLoading(true);
    try {
        const { data } = await supabase.from('app_settings').select('value').eq('key', 'car_prices_html').maybeSingle();
        if (data) {
  ...
  const handleSave = async () => {
      try {
          await supabase.from('app_settings').upsert({ key: 'car_prices_html', value: editContent });
          setContent(editContent);
          setIsEditing(false);
          alert("Đã cập nhật bảng giá xe!");
  ...
  return (
    <div className="w-full px-4 space-y-6 pb-20">
      <div className="flex justify-between items-center">
          <div>
              <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2"><CarFront className="text-primary-600" /> Bảng Giá & Thông tin Xe</h1>
      ...
      {!content && !isEditing && (
          <div className="bg-blue-50 p-4 rounded-xl border border-blue-200 text-blue-800 flex items-center gap-3">
              <Info size={24} />
              <p>Chưa có thông tin bảng giá xe được cập nhật.</p>
          </div>
      )}
