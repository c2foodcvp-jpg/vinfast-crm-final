...
import { Edit, Save, Loader2, Info, CarFront, Database, Copy, Terminal, AlertCircle } from 'lucide-react';

const CarPrices: React.FC = () => {
  ...
  const fetchContent = async () => {
    ...
    try {
        ...
        if (targetManagerId) {
            // New logic: Fetch from team_content table
  ...
  const handleSave = async () => {
      ...
      try {
          ...
          // Check if exists to decide upsert strategy (or use upsert with constraint)
          ...
          setIsEditing(false);
          alert("Đã cập nhật bảng giá xe cho Team!");
      } catch (e: any) { 
          if (e.code === '42P01') {
  ...
  return (
    <div className="w-full px-4 space-y-6 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
              <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2"><CarFront className="text-primary-600" /> Bảng Giá & Thông tin Xe (Team)</h1>
              <p className="text-gray-500">Thông tin giá niêm yết, khuyến mãi xe và thông số kỹ thuật.</p>
          </div>
      ...
      {!content && !isEditing && (
          <div className="bg-blue-50 p-4 rounded-xl border border-blue-200 text-blue-800 flex items-center gap-3">
              <Info size={24} />
              <p>Chưa có thông tin bảng giá xe được cập nhật cho Team của bạn.</p>
          </div>
      )}
      ...
      {isEditing ? (
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-4 animate-fade-in">
              ...
              <textarea 
                ...
                placeholder="Nhập mã HTML bảng giá xe... Ví dụ: <table>...</table>"
              ></textarea>
          ...
          <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 min-h-[400px]">
              {content ? (
                  <div className="prose max-w-none prose-img:rounded-xl prose-a:text-primary-600 prose-headings:text-gray-900" dangerouslySetInnerHTML={{ __html: content }} />
              ) : (
