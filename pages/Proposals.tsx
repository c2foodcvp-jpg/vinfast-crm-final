...
import { 
  ...
  AlertTriangle, ArrowRight, ShieldCheck, X
} from 'lucide-react';
      ...
      try {
          if (type === 'approve') {
              ...
              if (proposal.type === 'demo_car') {
                  ...
                  // A. Subtract from Team Fund (Expense) -> Reduces Pool
                  await supabase.from('transactions').insert([{
                      user_id: proposal.user_id, // Who borrowed
                      user_name: proposal.user_name,
                      type: 'expense',
                      amount: proposal.amount,
                      reason: `Phí mượn xe Demo (${carData.car_name})`,
                      status: 'approved',
                      approved_by: userProfile?.id
                  }]);
                  
                  // B. Add to Owner (Personal Bonus) -> Increases Owner's Income ONLY (Doesn't affect pool revenue)
                  const owner = allProfiles.find(p => p.id === carData.owner_id);
                  await supabase.from('transactions').insert([{
                      user_id: carData.owner_id,
                      user_name: owner?.full_name || 'Owner',
                      type: 'personal_bonus', // NEW TYPE
                      amount: proposal.amount,
