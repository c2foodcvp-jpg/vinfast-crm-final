...
const CustomerDetail: React.FC = () => {
  ...
  const fetchEmployees = async () => { 
      try { 
          ...
          // This allows "Change Sales" to list potential targets even if RLS hides them from view list
  ...
  const fetchCustomerData = async () => {
    try {
      ...
      // If user is Assigned Sales Rep (Robust & Fuzzy check)
          ...
          // Check exact match, or if one contains the other (e.g. "Nguyen Van A" vs "Van A")
  ...
  // 1. Is User the Assigned Sales Rep? (Robust & Fuzzy Check)
  ...
  // 2. Determine "Edit" Permission
  const canEdit = useMemo(() => {
      // Allow Admin / Mod / Creator / Sales Rep / Shared with Edit
      if (isAdmin || isMod) return true;
      if (customer?.creator_id === userProfile?.id) return true;
      if (isAssignedRep) return true; // Force True if assigned rep
  ...
  // 3. Can Share? (Admin, Mod, Creator, or Sales Rep)
  ...
  // 5. Users List for sharing and changing sales
  const availableUsersToShare = useMemo(() => {
      ...
      if (isMod) {
          return targets.filter(e => e.manager_id === userProfile.id || (myManagerId && e.id === myManagerId) || e.role === UserRole.ADMIN );
      }
      return targets.filter(e => (myManagerId && e.manager_id === myManagerId) || (myManagerId && e.id === myManagerId) || e.role === UserRole.ADMIN );
  ...
  const availableUsersToChange = useMemo(() => {
      ...
      // For standard users, show all available targets to request transfer
      return targets;
  }, [employees, userProfile, isAdmin, isMod]);
